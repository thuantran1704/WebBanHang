...
public class CartController {
	...
	public KhachHang getUserFromID(int id) {
		Session session = factory.getCurrentSession();
	     Criteria crit = session.createCriteria(KhachHang.class);
	...
	public String update(ModelMap model,@PathVariable int id, @RequestParam("soLuong") int soluong) {
		...
		ct.setSoTien(ct.getsanpham().getGiaSP()*ct.getSoLuong());
		Session session = factory.getCurrentSession();
		session.update(ct);
	...
	public String confirm(ModelMap model, @PathVariable int maHD, @PathVariable int tong) {
		...
		hd.setSoTien(tong);
		String tu="Admin_ThuanTran";
		KhachHang user=getUserFromID(hd.getMaKH());
